...
# ========== 4. Peeling Method ==========
def peeling_method(df, k=k, epsilon=epsilon, delta=delta):
    filtered = df[df['income'] == '>50K']
    grouped = filtered.groupby('feature').size()
    values = grouped.values.astype(float).copy()
    labels = grouped.index.values.copy()

    epsilon_prime = epsilon / (2 * np.sqrt(2 * k * np.log(1 / delta)))
